...
V3ElaborateFSMCmd::usage(const bool& verbose) const {
   ...
   Msg(MSG_IFO) << "                     [-Output <(string outputIndex)>] [-CONFIRM]" << endl;
   if (verbose) {
      ...
      Msg(MSG_IFO) << "       (string outputIndex): The index of output that represents the bad signal of a safety property." << endl;
      Msg(MSG_IFO) << "       -Input              : Indicate that the following token is the name for the input file." << endl;
      Msg(MSG_IFO) << "       -Output             : Indicate that the following token is the index of an output." << endl;
      Msg(MSG_IFO) << "       -Name               : Indicate that the following token is the name for FSMs." << endl;
      Msg(MSG_IFO) << "       -CONFIRM            : Self checking if the extraction is successful." << endl;
   }
}
...
//             [-Time <(unsigned maxTime)>] [-SCC | -NONE] [-CONFIRM]
//----------------------------------------------------------------------
V3CmdExecStatus
V3ExtractFSMCmd::exec(const string& option) {
   ...
   for (size_t i = 0; i < n; ++i) {
      ...
      if (v3StrNCmp("-SCC", token, 4) == 0) {
         if (scc || none) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else if (nameON) return V3CmdExec::errorOption(CMD_OPT_MISSING, "(string fsmName)");
...
V3ExtractFSMCmd::usage(const bool& verbose) const {
   Msg(MSG_IFO) << "EXTract FSM [-Name <(string fsmName)>] [-Output <(string outputIndex)>]" << endl;
   Msg(MSG_IFO) << "            [-Time <(unsigned maxTime)>] [-SCC | -NONE] [-CONFIRM]" << endl;
   if (verbose) {
      Msg(MSG_IFO) << "Param: (string fsmName)    : The name of FSMs to be extracted." << endl;
      Msg(MSG_IFO) << "       (string outputIndex): The index of output that represents the bad signal of a safety property." << endl;
      Msg(MSG_IFO) << "       (unsigned maxTime)  : The runtime limit for extraction." << endl;
      Msg(MSG_IFO) << "       -Time               : Indicate that the following token is the maximum runtime limit." << endl;
      Msg(MSG_IFO) << "       -Output             : Indicate that the following token is the index of an output." << endl;
      Msg(MSG_IFO) << "       -Name               : Indicate that the following token is the name for FSMs." << endl;
