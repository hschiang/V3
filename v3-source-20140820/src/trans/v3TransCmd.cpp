...
V3ExpandNtkCmd::usage(const bool& verbose) const {
   Msg(MSG_IFO) << "Usage: EXPand NTk <(unsigned cycle)> [-Initial]" << endl;
   if (verbose) {
      Msg(MSG_IFO) << "Param: (unsigned cycle): The number of time-frames to be considered." << endl;
      Msg(MSG_IFO) << "       -Initial        : Set initial state values at the first timeframe." << endl;
   }
}
...
V3FlattenNtkCmd::usage(const bool& verbose) const {
   Msg(MSG_IFO) << "Usage: FLAtten NTk [(unsigned level)]" << endl;
   if (verbose) {
      Msg(MSG_IFO) << "Param: (unsigned level): The number of hierarchical levels to be flattened. (default = inf)" << endl;
   }
}
