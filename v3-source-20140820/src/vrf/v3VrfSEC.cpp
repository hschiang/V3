...
V3VrfSEC::startVerify(const uint32_t& p) {
   ...
   while (_maxDepth >= _secLastDepth) {
      ...
      while (_secChecker) {
         ...
         _secChecker->setMaxTime(V3NtkUD);
...
V3VrfSEC::speculativeReduction(const uint32_t& cycle) {
   ...
   while (_secCandidate.size()) {
      ...
      uint32_t j = cycle; cexList.clear();
      for (uint32_t i = 0; i < eqNets.size(); ++i) {
         solver->addBoundedVerifyData(eqNets[i], j); solver->simplify();
         solver->assumeRelease(); solver->assumeProperty(eqNets[i], true, j);
...
V3VrfSEC::isValidInvariant(const uint32_t& x) {
   ...
   checker->setMaxTime(V3NtkUD); checker->setMaxDepth(V3NtkUD); checker->verifyProperty(0);
