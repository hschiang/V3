...
vrfRestart: 
   ...
   while (1) {
      // Check Time Bounds
      if (_maxTime < ((clock() - runtime) / CLOCKS_PER_SEC)) break;
      // Check Memory Bound
      ...
      while (true) {
         ...
         double itptime = clock();
         result = blockNewCube();  // SAT (T ^ Ri')
         cerr << "ITPTIME = " << (clock() - itptime) / CLOCKS_PER_SEC << endl;
         ...
         if (1 == result) {  // Spurious Cex Found
            ...
            // JOURNAL STATISTICS ONLY
            cerr << "ISIZE: " << _itpFrame.size() - 1 << endl;
      ...
      cerr << "KTIME = " << (clock() - ktime) / CLOCKS_PER_SEC << endl;
...
V3VrfCITP::initializeITPSolver() {
   ...
   // Set Initial State to Solver
   for (uint32_t i = 0, j = 0; i < _vrfNtk->getLatchSize(); ++i) _itpITPSvr->addBoundedVerifyData(_vrfNtk->getLatch(i), j);
   uint32_t j = 0; assert (1 == _itpBad->getState().size()); _itpITPSvr->addBoundedVerifyData(_itpBad->getState()[0], j);
   for (uint32_t i = 0, j = 1; i < _vrfNtk->getLatchSize(); ++i) _itpITPSvr->addBoundedVerifyData(_vrfNtk->getLatch(i), j);
   // Check Shared Invariants
...
V3VrfCITP::blockNewCube() {
   ...
   // JOURNAL STATISTICS ONLY
   cerr << "CUBESIZE: " << _itpFrame.back()->getCubeList().size() << endl;
