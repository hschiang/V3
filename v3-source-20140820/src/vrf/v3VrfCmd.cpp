...
V3SetSolverCmd::exec(const string& option) {
   ...
   bool dft = false, msat = false, boolector = false;
   
   size_t n = options.size();
   for (size_t i = 0; i < n; ++i) {
      const string& token = options[i];
      if (v3StrNCmp("-Default", token, 2) == 0) {
         if (dft || msat || boolector) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else dft = true;
      }
      else if (v3StrNCmp("-MSat", token, 3) == 0) {
         if (dft || msat || boolector) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else msat = true;
      }
   ...
   if (!(dft || msat || boolector)) dft = true;

   // Set Verification Solvers
   if (msat) V3VrfBase::setDefaultSolver(V3_SVR_MINISAT);
...
V3SetSolverCmd::usage(const bool& verbose) const {
   Msg(MSG_IFO) << "Usage: SET SOlver [|-Default | -MSat | -Boolector]" << endl;
   if (verbose) {
      Msg(MSG_IFO) << "Param: -Default  : Enable default solver. (default = minisat)" << endl;
      Msg(MSG_IFO) << "       -MSat     : Enable MiniSat as the active solver." << endl;
...
V3SetPropertyCmd::exec(const string& option) {
   ...
   for (size_t i = 0; i < n; ++i) {
      ...
      if (v3StrNCmp("-Safety", token, 2) == 0) {
         ...
         else safe = true; invAON = invCON = fairON = false;
      }
      else if (v3StrNCmp("-Liveness", token, 2) == 0) {
         if (safe || live) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else if (nameON) return V3CmdExec::errorOption(CMD_OPT_MISSING, "(string propertyName)");
         else if (outputON) return V3CmdExec::errorOption(CMD_OPT_MISSING, "(string outputIndex)");
         else live = true; invAON = invCON = fairON = false;
      }
         ...
         else output = outputON = true; invAON = invCON = fairON = false;
      }
   ...
   if (safe && invNameList[2].size()) {
      Msg(MSG_ERR) << "Unexpected Fairness Constraints for Safety Properties !!" << endl;
      return CMD_EXEC_DONE;
   }
   ...
   if (handler) {
      ...
      if (handler->existProperty(propertyName))
         Msg(MSG_ERR) << "Property \"" << propertyName << "\" has Already Exists !!" << endl;
      else {
         ...
         else f = new V3LTLFormula(handler, V3NetId::makeNetId(0), safe, propertyName);
      }
      ...
      if (f && f->isValid()) {
         ...
         if (k == 3) {
            ...
            Msg(MSG_IFO) << "Property " << propertyName << " has been set successfully !!" << endl;
         }
         else delete p;
      }
      else {
...
void
V3SetPropertyCmd::usage(const bool& verbose) const {
   Msg(MSG_IFO) << "Usage: SET PRoperty [-Name <(string propertyName)>] [|-Safety |-Liveness]" << endl;
   Msg(MSG_IFO) << "                    [-Output <(unsigned outputIndex)>]" << endl;
   ...
   if (verbose) {
      ...
      Msg(MSG_IFO) << "       -Output               : Indicate the follwoing token is the index of a bad output." << endl;
      ...
      Msg(MSG_IFO) << "       -Safety               : Set a safety property." << endl;
      Msg(MSG_IFO) << "       -Liveness             : Set a liveness property." << endl;
   }
}

void
V3SetPropertyCmd::help() const {
   Msg(MSG_IFO) << setw(20) << left << "SET PRoperty: " << "Set Properties on Current Network." << endl;
}
...
V3UMCVrfCmd::exec(const string& option) {
   ...
   bool maxDON = false, preDON = false, incDON = false;
   uint32_t maxDepth = 0, preDepth = 0, incDepth = 0;
   bool noProve = false, noFire = false, unique = false;
...
V3ITPVrfCmd::exec(const string& option) {
   ...
   bool block = false, blockON = false;
   bool recycle = false;
...
V3ITPVrfCmd::usage(const bool& verbose) const {
   ...
   Msg(MSG_IFO) << "                  [-Reverse] [-Increment] [-Force] [-RECycle] [-Block (unsigned badCount)]" << endl;
   if (verbose) {
      Msg(MSG_IFO) << "Param: (string propertyName): The name of a property to be verified." << endl;
      Msg(MSG_IFO) << "       (unsigned MaxDepth)  : The upper bound of time-frames to be reached." << endl;
      ...
      Msg(MSG_IFO) << "       -Reverse             : Enables the reversed implementation of NewITP." << endl;
...
V3ITPVrfCmd::help() const {
   Msg(MSG_IFO) << setw(20) << left << "VERify ITP: " << "Perform Interpolation-based Model Checking Algorithm NewITP." << endl;
}
...
V3SECVrfCmd::exec(const string& option) {
   ...
   bool umc = false, mpdr = false, ipdr = false;
   ...
   for (size_t i = 0; i < n; ++i) {
      ...
      if (v3StrNCmp("-Max-depth", token, 2) == 0) {
         ...
         else if (ipdr || mpdr) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else if (maxDON) return V3CmdExec::errorOption(CMD_OPT_MISSING, "(unsigned MaxDepth)");
         else umc = true;
      }
      else if (v3StrNCmp("-IPDR", token, 5) == 0) {
         if (ipdr) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else if (umc || mpdr) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else if (maxDON) return V3CmdExec::errorOption(CMD_OPT_MISSING, "(unsigned MaxDepth)");
         else ipdr = true;
      }
      else if (v3StrNCmp("-MPDR", token, 5) == 0) {
         if (mpdr) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else if (umc || ipdr) return V3CmdExec::errorOption(CMD_OPT_EXTRA, token);
         else if (maxDON) return V3CmdExec::errorOption(CMD_OPT_MISSING, "(unsigned MaxDepth)");
         else mpdr = true;
      }
   ...
   if (handler) {
      if (propertyName.size()) {
         if (handler->existProperty(propertyName)) {
            ...
            if (umc) { secChecker = new V3VrfUMC(checker->getSECHandler()); assert (secChecker); }
            else if (mpdr) {
...
//              [-Inc-depth (unsigned IncDepth)] [| -UMC | -IPDR | -MPDR]
//----------------------------------------------------------------------
V3CmdExecStatus
V3KLiveVrfCmd::exec(const string& option) {
   ...
   bool umc = false, ipdr = false, mpdr = false;
