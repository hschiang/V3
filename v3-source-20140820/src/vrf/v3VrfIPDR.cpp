...
V3VrfIPDR::initializeSolver(const uint32_t& d, const bool& isReuse) {
   if (profileON()) _initSvrStat->start(); uint32_t j = 0;
   ...
   for (uint32_t i = 0; i < _vrfNtk->getLatchSize(); ++i) _pdrSvr[d]->addBoundedVerifyData(_vrfNtk->getLatch(i), j);
   // Set p to this Frame if it is NOT the Last Frame
   _pdrSvr[d]->addBoundedVerifyData(_pdrBad->getState()[0], j);
...
V3VrfIPDR::addCubeToSolver(const uint32_t& frame, const V3NetVec& state, const uint32_t& d) {
   ...
   for (uint32_t i = 0, j = d; i < state.size(); ++i) {
      assert (state[i].id < _vrfNtk->getLatchSize());
      _pdrSvr[frame]->addBoundedVerifyData(_vrfNtk->getLatch(state[i].id), j);
   }
}
void
