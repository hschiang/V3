...
V3VrfMPDR::initializeSolver() {
   ...
   for (uint32_t i = 0; i < _vrfNtk->getLatchSize(); ++i) _pdrSvr->addBoundedVerifyData(_vrfNtk->getLatch(i), j);
...
V3VrfMPDR::addCubeToSolver(const V3NetVec& state, const uint32_t& depth) {
   ...
   for (uint32_t i = 0, j = depth; i < state.size(); ++i) {
      assert (state[i].id < _vrfNtk->getLatchSize());
      _pdrSvr->addBoundedVerifyData(_vrfNtk->getLatch(state[i].id), j);
   }
}
...
V3VrfMPDR::getInitialObligation() {  // If SAT(R ^ T ^ !p)
   ...
   uint32_t j = 0; _pdrSvr->addBoundedVerifyData(state[0], j);
