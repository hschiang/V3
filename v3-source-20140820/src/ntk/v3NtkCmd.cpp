...
V3WriteRTLCmd::usage(const bool& verbose) const {
   Msg(MSG_IFO) << "Usage: WRIte Rtl <(string fileName)> [-Symbol] [-Initial]" << endl;
   if (verbose) {
      ...
      Msg(MSG_IFO) << "       -Initial         : Enable specifying initial states of a network." << endl;
   }
}
...
V3WriteBTORCmd::help() const {
   Msg(MSG_IFO) << setw(20) << left << "WRIte Btor: " << "Write BTOR Designs." << endl;
}
...
V3PrintNtkCmd::usage(const bool& verbose) const {
   ...
   if (verbose) {
      ...
      Msg(MSG_IFO) << "       -CombLoops  : Print combinational loops." << endl;
      Msg(MSG_IFO) << "       -Floating   : Print floating nets." << endl;
      Msg(MSG_IFO) << "       -Unreachable: Print unreachable nets." << endl;
   }
}
...
V3RefNtkCmd::exec(const string& option) {
   ...
   if (handler) {
      if (ntkId >= v3Handler.getHandlerCount()) 
         ...
         updateModule(handler->getNtk(), handler->getNtk()->getModule(index), v3Handler.getHandler(ntkId), false);
      }
   }
   else Msg(MSG_ERR) << "Empty Ntk !!" << endl;
   return CMD_EXEC_DONE;
