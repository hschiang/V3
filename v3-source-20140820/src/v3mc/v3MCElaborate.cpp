...
V3NtkHandler* const elaborateLivenessNetwork(V3NtkHandler* const handler, const V3UI32Vec& poList, V3NetTable& invConstr, V3NetTable& fairConstr) {
   ...
   // Strengthen Fairness Constraints
   V3NetVec fairList; fairList.clear();
   for (uint32_t i = 0; i < fairConstr.size(); ++i)
      fairList.insert(fairList.end(), fairConstr[i].begin(), fairConstr[i].end());
   V3NetVec invList; invList.clear(); if (invConstr.size()) invList = invConstr[0];
   for (uint32_t i = 1; i < invConstr.size(); ++i) if (invConstr[0] != invConstr[i]) { invList.clear(); break; }
   strengthenFairnessConstraints(copyHandler, invList, fairList);
   for (uint32_t i = 0, k = 0; i < fairConstr.size(); ++i)
      for (uint32_t j = 0; j < fairConstr[i].size(); ++j, ++k) fairConstr[i][j] = fairList[k];
