...
V3MCResource::reallocNumOfCores() {
   ...
   //if (v3MCInteractive) {
      ...
      for (uint32_t i = 0; i < V3MC_TYPE_TOTAL; ++i)
         cerr << V3MCMainTypeStr[i] << " = " << _usedCores[i] << "/" << _numOfCores[i] << "  ";
      cerr << endl;
...
void* startChecker(void* param) {
   ...
   V3UI32Vec bound; p->_sharedBound->getBound(bound); uint32_t unsolvedSize = 0;
   for (uint32_t i = 0; i < bound.size(); ++i) if (V3NtkUD != bound[i]) ++unsolvedSize;
   // Start Checker
   if (unsolvedSize) {
   ...
   if (checker) {
      if (v3MCInteractive) {
         V3VrfShared::printLock();
         cerr << V3MCMainTypeStr[p->_type] << " Creates " << V3MCEngineTypeStr[type]
            << " with Unsolved Parameter = " << unsolvedSize << "." << endl;
         V3VrfShared::printUnlock();
      }
      ...
      V3UI32Vec lastDepth(pNtk->getNtk()->getOutputSize(), 0);
      for (uint32_t round = 1; round <= 64; round *= 2) {
         // Check Memory Bound
         if (p->_sharedMem && !p->_sharedMem->isMemValid()) break;
         // Get Number of Unsolved Properties
         p->_sharedBound->getBound(bound); unsolvedSize = 0;
         for (uint32_t i = 0; i < bound.size(); ++i) if (V3NtkUD != bound[i]) ++unsolvedSize;
         if (!unsolvedSize) break; uint32_t depthBound = 0;
         // Set Bound to Checker
         const bool fixedMaxDepth = (V3MC_SEC_BMC == type) || (V3MC_SEC_UMC == type) ||
            ...
            (V3MC_SIM == type);
         if (!fixedMaxDepth) {
            uint32_t maxDepth = (unsolvedSize > 10000) ? 3 : (unsolvedSize > 1000) ? 5 :
               (unsolvedSize > 100) ? 8 : (unsolvedSize > 10) ? 10 :
               (unsolvedSize > 1) ? 20 : V3NtkUD;
            if (V3NtkUD == depthBound) break;
            if (!V3SafeMult(round, maxDepth, depthBound)) depthBound = V3NtkUD;
            checker->setMaxDepth(depthBound);
         }
         ...
         while (true) {
            ...
            if (v3MCInteractive) {
               V3VrfShared::printLock();
               cerr << V3MCMainTypeStr[p->_type] << "/" << V3MCEngineTypeStr[type]
                  << " Start Verifying PO[" << property << "] At Round = " << round;
               if (!fixedMaxDepth) cerr << " with MaxDepth = " << depthBound; cerr << "." << endl;
               V3VrfShared::printUnlock();
            }
            // Start Verification
            ...
            checker->setIncLastDepthToKeepGoing((V3MC_LIVE == p->_type) ? 0 : lastDepth[property]);
            ...
            if (fixedMaxDepth && !decided) break;
            if (p->_sharedMem && !p->_sharedMem->isMemValid()) break;
            lastDepth[property] = depthBound; tried[property] = true;
         }
         if (fixedMaxDepth) break;
      }
   ...
   if (v3MCInteractive) {
      V3VrfShared::printLock();
      cerr << V3MCMainTypeStr[p->_type] << "/" << V3MCEngineTypeStr[type] << " Terminates." << endl;
      V3VrfShared::printUnlock();
   }
}
...
void* startVerificationMain(void* param) {
   ...
   if (v3MCInteractive) {
      V3VrfShared::printLock();
      cerr << V3MCMainTypeStr[p->_type] << " Created : " << "NetSize = " << handler->getNtk()->getNetSize() << ", "
           << "NumOfProperty = " << handler->getNtk()->getOutputSize() << endl;
      V3VrfShared::printUnlock();
   }
   // Reorder Properties
   //reorderPropertyByNetId(handler, p->_constr, V3MC_LIVE == p->_type);
   reorderPropertyByCOI(handler, p->_constr, V3MC_LIVE == p->_type);
   if (v3MCInteractive) {
      V3VrfShared::printLock();
      cerr << V3MCMainTypeStr[p->_type] << " Properties: " << endl;
      for (uint32_t i = 0; i < handler->getNtk()->getOutputSize(); ++i) cerr << handler->getOutputName(i) << " ";
      cerr << endl;
      V3VrfShared::printUnlock();
   }
   ...
   while (true) {
      ...
      if (v3MCInteractive) {
         ...
         V3VrfShared::printLock();
         cerr << V3MCMainTypeStr[p->_type] << " REPORT : "
      ...
      if (occupied < available) {
         ...
         if (v3MCInteractive) {
            V3VrfShared::printLock();
            cerr << V3MCMainTypeStr[p->_type] << " CREATE : "
         ...
         for (uint32_t i = 0; i < bound.size(); ++i) {
            ...
            if (result[i].isCex()) {
               ...
               if (checkResult > 0) {
                  ...
                  if (v3MCInteractive) {
                     V3VrfShared::printLock();
                     cerr << V3MCMainTypeStr[p->_type] << " Reports Result for Property (" << i << ") "
                  ...
                  if (v3MCInteractive) {
                     V3VrfShared::printLock();
                     cerr << V3MCMainTypeStr[p->_type] << " Confirms SPURIOUS Result for Property (" << i << ") "
                          << handler->getOutputName(i) << " At Iter = " << iter << "!!" << endl;
                     V3VrfShared::printUnlock();
                  }
                  ++unsolved;
