...
V3NtkHandler* const readProperty(V3NtkHandler* const handler, const string& fileName) {
   ...
   Msg(MSG_IFO) << "Totally " << safeSize << " Safety, " << liveSize << " Liveness, "
                << invcSize << " Invariant Constraints, " << fairSize << " Fairness Constraints Added." << endl;
...
void writeProperty(V3NtkHandler* const handler, const string& fileName, const bool& l2s, const bool& invc2Prop) {
   ...
   assert (propList.size() == prop.size()); assert (prop.size() == name.size());
...
void startVerificationFromProp(V3NtkHandler* const handler, const uint32_t& numOfCores, const double& maxTime, const double& maxMemory) {
   ...
   if (v3MCInteractive) {
      ...
      cerr << "Network = " << handler->getNtkName() << ", Net Size = " << ntk->getNetSize() << endl;
      cerr << "User Settings: NumOfCores = " << numOfCores << ", MaxTime = " << maxTime << " sec, "
   ...
   if (pDistrib[V3MC_SAFE]) {
      V3VrfSharedBound* const sharedBound = new V3VrfSharedBound(pDistrib[V3MC_SAFE], pDistrib[V3MC_SAFE] == 1);
      ...
      V3NtkHandler* const invExtNtk = extractFSMInvariants(simpHandler, 10.00); assert (invExtNtk);
      // Create Safety Checking Main Parameters
      V3VrfSharedNtk* const sharedNtk = new V3VrfSharedNtk(); assert (sharedNtk);
      if (invExtNtk != simpHandler) delete simpHandler;
      pS._handler = invExtNtk;
      //pS._constr = invList;
      pS._type = V3MC_SAFE;
      ...
      pS._result = result;
      if (!liveSize) { delete elabNtk; startVerificationMain((void*)&pS); }
   ...
   if (liveSize) {
      ...
      if (pDistrib[V3MC_LIVE]) {
         ...
         pK._result = result;
         if (pDistrib[V3MC_LIVE] == pDistrib[V3MC_L2S]) { delete elabNtk; startVerificationMain((void*)&pK); }
